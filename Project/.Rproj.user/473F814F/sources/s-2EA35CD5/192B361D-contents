...
x1<-seq(-5,20,length=41) # generating the vector series x1
x2<-seq(0,15, length=41) # copying x1 to x2
...
contour(x = x1, y = x2, z1, col = 'red', drawlabel=TRUE, main="Decision: Tiger or Greasy")
contour(x = x1, y = x2, z2, col = 'blue', drawlabel=TRUE, add=TRUE)
contour(x = x3, y = x4, z3, col = 'black',levels = 0, drawlabel=FALSE, add=TRUE)
legend ( par('usr')[1], par('usr')[4], xjust = 0, 
         c('Tiger', "Greasy"), 
         lwd = c(1, 1), lty = c(1, 1), 
         col = c('red', 'blue'))
